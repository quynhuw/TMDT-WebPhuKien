...
const PaymentPage = () => {
  ...
  const handleApplyDiscount = () => {
    ...
    setTotalOrder(calTotalOrder() - discount);
  };
  ...
  const handleChangeAddress = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (listAddress.length > 0) {
      const addressJson = e.target.value;
  ...
  const handleSubmit = () => {
    ...
    if (payment == 1) {
      // api TP => link
      // open link => get orderDetails from localStorage
      // if success => call api write db => show page success
      // else => show message
    }

    const orderDetails = {
      payment_type_id: payment, // 1 là người bán trả (người mua đã thanh toán qua paypal), 2 là người mua trả
      note: note,
